...
def download(class_name, args):
    ...
    with open(new_csv) as dataset:
        ...
        allNames = os.listdir(path = dst_dir)
        print(allNames)
        for row in reader:
            ''' old Code meh
            # print command for debugging
            print("ffmpeg -ss " + str(row[1]) + " -t 10 -i $(youtube-dl -f bestaudio https://www.youtube.com/watch?v=" +
                       str(row[0]) + ") -ar " + str(DEFAULT_FS) + " -- \"" + dst_dir + "/" + str(row[0]) + "_" + row[1] + ".wav\"")
            os.system(("ffmpeg -ss " + str(row[1]) + " -t 10 -i $(youtube-dl -f bestaudio https://www.youtube.com/watch?v=" +
                       str(row[0]) + ") -ar " + str(DEFAULT_FS) + " -- \"" + dst_dir + "/" + str(row[0]) + "_" + row[1] + ".wav\""))
            New Code :D'''
            fileName = str(row[0]) + "_" + row[1] + ".wav";
            if fileName not in allNames:
                allNames.append(fileName)
                fileName = str(DEFAULT_FS) + " -- \"" + dst_dir + "/" + fileName
                os.system("youtube-dl -x --audio-format wav https://www.youtube.com/watch?v=" + row[0] + " --output \"currentFile.wav\"")
                if "currentFile.wav" in os.listdir():
                    downloadString = "ffmpeg -ss " + str(row[1]) + " -t 10 -i \"currentFile.wav\" -ar " + fileName;
